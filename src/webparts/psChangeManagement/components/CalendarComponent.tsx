...
const CalendarComponent = ({ props }) => {
  ...
  const getListData = (param) => {
    ...
    props.context.aadHttpClientFactory
      .getClient('https://api-fp-ps-cm.azurewebsites.net')
      .then((client: AadHttpClient): void => {
        client
          .get('https://api-fp-ps-cm.azurewebsites.net/api/ps-cm?call=' + param, AadHttpClient.configurations.v1)
      ...
      {showLoader ? <div className="msSpinner">
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} className="margin16px float-right">
            <Button variant="contained" id="pastevents" color="primary" onClick={showPastEvents}>
              Load older events?
       </Button>
          </Grid>
          <Grid item xs={12} >
